(function () {
  ...
  function refreshLockState() {
    ...
    const fullOv = ensureOverlay('gameFullOverlay', 'Game is full',
      'All four bombers are taken right now. This page will unlock if a spot opens up.');
  ...
  const existing = localStorage.getItem('bombbrawl.playerId');
  if (existing) { window.location.replace('/bombbrawl/play'); return; }

  const rejoinName = localStorage.getItem('bombbrawl.rejoinName');
  if (rejoinName) { nameInput.value = rejoinName; localStorage.removeItem('bombbrawl.rejoinName'); }
  ...
  socket.on('connect', function () {
    ...
    socket.emit('query:status', {}, function (status) {
      ...
      setGameFull(!!(status && typeof status.total === 'number' && typeof status.capacity === 'number' && status.total >= status.capacity));
    });
  });
  ...
  form.addEventListener('submit', function (e) {
    ...
    socket.emit('player:join', { playerId: pid, name: name }, function (res) {
      ...
      localStorage.setItem('bombbrawl.playerId', pid);
      localStorage.setItem('bombbrawl.playerName', res.player.name);
      window.location.replace('/bombbrawl/play');
    });
  });
})();
