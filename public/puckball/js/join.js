(function () {
  ...
  const existing = localStorage.getItem('puckball.playerId');
  if (existing) { window.location.replace('/puckball/play'); return; }

  const rejoinName = localStorage.getItem('puckball.rejoinName');
  if (rejoinName) { nameInput.value = rejoinName; localStorage.removeItem('puckball.rejoinName'); }
  ...
  form.addEventListener('submit', function (e) {
    ...
    socket.emit('player:join', { playerId: pid, name: name }, function (res) {
      ...
      localStorage.setItem('puckball.playerId', pid);
      localStorage.setItem('puckball.playerName', res.player.name);
      window.location.replace('/puckball/play');
    });
  });
})();
