(function () {
  ...
  const existing = localStorage.getItem('slingsoccer.playerId');
  if (existing) { window.location.replace('/slingsoccer/play'); return; }

  const rejoinName = localStorage.getItem('slingsoccer.rejoinName');
  if (rejoinName) { nameInput.value = rejoinName; localStorage.removeItem('slingsoccer.rejoinName'); }
  ...
  form.addEventListener('submit', function (e) {
    ...
    socket.emit('player:join', { playerId: pid, name: name }, function (res) {
      ...
      localStorage.setItem('slingsoccer.playerId', pid);
      localStorage.setItem('slingsoccer.playerName', res.player.name);
      window.location.replace('/slingsoccer/play');
    });
  });
})();
